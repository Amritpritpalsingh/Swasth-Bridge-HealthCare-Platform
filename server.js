...
/*************************************************
 * Load ENV + Connect DB FIRST
 *************************************************/
require("dotenv").config();
const connectDB = require("./utils/configs/db");

(async () => {
  await connectDB(); // ✅ MUST CONNECT BEFORE ANYTHING
  ...
  /*************************************************
   * Passport Config (AFTER DB)
   *************************************************/
  require("./utils/configs/passportConfig")(passport);
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("call-ended", ({ fromUser, targetUserId }) => {
      const targetSocket = userSockets[targetUserId];
      if (targetSocket) {
        io.to(targetSocket).emit("call-ended", fromUser);
      }
    });
